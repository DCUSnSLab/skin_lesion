...
def inference(opt,device,model):
    ...
    # str(format(classes.item(), '02'))
    return (str(format(classes.item(), '02')),conf.item())
...
if __name__ == '__main__':
    opt = argparse.Namespace(
        ...
        dir_path=str('/mnt/ISIC-Archive-Data/test2'),
        # gt=str('')
    )
    # print(opt.dir_path)
    ...
    for i, data in enumerate(path_list):
        ...
        if classes != lable_list[i]:
            print(classes, lable_list[i],conf)
            cnt+=1
        if conf>=0.9:
            modified_preds.append(classes)
            modified_label_list.append(lable_list[i])
    ...
    print("f1_score", f1_score(lable_list, pred
                               , average='macro'))
    print('*' * 50)
    print('modified',len(modified_preds))
    print(confusion_matrix(modified_label_list, modified_preds))
    print("accuracy", accuracy_score(modified_label_list, modified_preds))
    print("precision", precision_score(modified_label_list, modified_preds
                                       , average='macro'))
    print("recall", recall_score(modified_label_list, modified_preds
                                 , average='macro'))
    print("f1_score", f1_score(modified_label_list, modified_preds
                               , average='macro'))
